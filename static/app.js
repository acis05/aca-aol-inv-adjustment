...
function showResultPanel(summary = {}, results = []) {
  ...
  if (failedItems.length > 0) {
    const lines = [];
    lines.push("CATATAN GAGAL IMPORT INVENTORY ADJUSTMENT");
    lines.push("==================================");
    failedItems.forEach((x, idx) => {
      lines.push("");
      lines.push(`${idx + 1}. Adjustment: ${x.number || "-"} | Tanggal: ${x.transDate || "-"}`);
...
if ($("btnBuild")) {
  $("btnBuild").onclick = async () => {
    try {
      ...
      const res = await postForm("/api/build-inventory-adjustment", fd);

      builtPayload = res.payload;

      setSummary(`File siap diimport: ${res.summary.transactions} adjustment, ${res.summary.lines} item.`);
      showResultPanel({
      ...
      showNotify(
        ...
        "File siap diimport",
        renderSimpleMessage([
          `Jumlah adjustment: ${res.summary.transactions}`,
...
if ($("btnImport")) {
  $("btnImport").onclick = async () => {
    try {
      ...
      setSummary("Sedang mengimport Inventory Adjustment ke Accurate...");
      const res = await postJson("/api/import-inventory-adjustment", { payload: builtPayload }, true);
      ...
      setSummary(hasFailed ? "Import selesai, ada adjustment yang perlu diperbaiki." : "Import berhasil. Semua Inventory Adjustment berhasil masuk ke Accurate.");

      showNotify(
        hasFailed ? "error" : "success",
        hasFailed ? "Import selesai dengan catatan" : "Import berhasil",
        renderSimpleMessage([
          `Total adjustment: ${summary.total || 0}`,
      ...
      if (results.length > 0) {
        showResultPanel(summary, results);
        setSummary("Import selesai, ada adjustment yang perlu diperbaiki.");
        showNotify(
          "error",
